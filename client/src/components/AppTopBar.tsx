...
export default function AppTopBar({ eyebrow, title, description, actionLabel, onAction, actionDisabled = false }: { eyebrow: string; title: ReactNode; description?: string; actionLabel?: string; onAction?: () => void; actionDisabled?: boolean }) {
  ...
  return (
    <header className="flex flex-col gap-6 border-b border-[#20403e] px-5 py-6 sm:px-8 lg:flex-row lg:items-start lg:justify-between lg:px-12">
      <div>
        <div className="micro-label mb-3 flex items-center gap-2 text-[#a4d595]"><span className="h-1.5 w-1.5 rounded-full bg-[#c8ff4f] shadow-[0_0_14px_#c8ff4f]" />{eyebrow}</div>
        <h1 className="display-font max-w-2xl text-4xl leading-[0.95] tracking-tight text-[#f2f4eb] sm:text-5xl">{title}</h1>
        {description && <p className="mt-4 max-w-xl text-sm leading-6 text-[#7fa194]">{description}</p>}
      </div>
      <div className="flex items-center gap-2 lg:pt-1">
        ...
        {actionLabel && <Button onClick={onAction} disabled={actionDisabled} className="action-button ml-2 h-10 rounded-lg bg-[#c8ff4f] px-4 text-xs font-bold text-[#112119] hover:bg-[#ddff8b]"><Sparkles className="mr-2 h-3.5 w-3.5" />{actionDisabled ? "Processando…" : actionLabel}</Button>}
      </div>
    </header>
  );
}
